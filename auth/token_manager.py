...
async def update_tokens_cookie(uid, response):
    ...
    response.set_cookie(config.JWT_ACCESS_COOKIE_NAME, a_token)
    response.set_cookie(config.JWT_REFRESH_COOKIE_NAME, r_token)
    return {
...
async def refresh_tokens(r_token, a_token, response):
    uid = await get_id_from_access_token(a_token=r_token)
